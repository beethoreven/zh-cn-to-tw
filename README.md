...
- **本月使用量**（Claude）：記錄每次呼叫實際用掉的 input/output token
  ...
  參考，會跟台銀牌告價有落差）。統計週期預設是台灣時區的日曆月，
  起算日可用 `USAGE_BILLING_CYCLE_START_DAY` 調整——**這跟 Claude
  Code/Claude.ai 的訂閱方案是兩套獨立帳務系統**，Anthropic API
  Console 帳號實際的計費週期要申請後才看得到，不一定是日曆月。
  這個台幣金額本質上是參考估計值，不是查詢 Anthropic 帳戶的真實帳單。
...
- `GET /api/usage/monthly` — Claude 系列 model 本月 token 用量與估計台幣費用
...
- [x] Stage 2：二次校對（用詞/錯字），結構化清單 + 一鍵套用，手動觸發
  ...
  - [x] 本月使用量（Claude token 數 + 估計台幣費用）
- [x] 最左側獨立一欄「阿舍老師的叮嚀」內容區塊：純文字，內容來源是
  `zh-cn-to-tw-backend/content/tutor_arthur_announce.txt`，跟著程式碼進版控，要改
  內容直接編輯這個檔案再 push，不需要動程式碼或另外做後台介面
